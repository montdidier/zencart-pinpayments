                  ...
                  if ($order->info['payment_module_code']) {
                      if (file_exists(DIR_FS_CATALOG_MODULES . 'payment/' . $order->info['payment_module_code'] . '.php')) {
                          ...
                          if (method_exists($module, '_doRefund')) {
                              $refund_amount = '';
                              if ($_POST['typeRefund']=='partial')
                              {
                                  $refund_amount = $_POST['doPartialRefund'];
                              }
                              else
                                  $refund_amount = $order->info['total'];

                              $module->_doRefund($oID, $_POST['typeRefund'], $refund_amount, $_POST['akey'], $order->info['currency'] );
    ...
    <!-- do refund with pin payments -->
      ...
      <td><table border="0" cellspacing="0" cellpadding="2" class="noprint">
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="2">
                <tr>
                  <td class="main"></strong> <?php echo zen_draw_checkbox_field('doFullRefund', "true", false); ?></td>
                  <td class="main"></strong> <input type="button" name="exeFullRefund" value="do full refund" onclick="javascript: exeOrderRefund('full');" /></td>
                </tr>
              </table></td>
            <td valign="top"></td>
          </tr>
          <tr>
            <td><table border="0" cellspacing="0" cellpadding="2">
                <tr>
                  <td class="main"></strong> <?php echo zen_draw_input_field('doPartialRefund', ""); ?></td>
                  <td class="main"></strong> <input type="button" name="exePartialRefund" value="do partial refund" onclick="javascript: exeOrderRefund('partial');" /></td>
                </tr>
      ...
      <script type="text/javascript">
          function exeOrderRefund(typeRefund)
          {
              if (typeRefund=='partial'){
                 if (parseFloat(document.refund.doPartialRefund.value) <= 0 || isNaN(parseFloat(document.refund.doPartialRefund.value))){
                     alert("Please enter an amount must be greater than zero");
                     return false;
                 } else {
                     document.refund.typeRefund.value = 'partial';
                     document.refund.submit();
                 }
              } else
              {
                  if (document.refund.doFullRefund.checked){
                      document.refund.typeRefund.value = 'full';
                      document.refund.submit();
                  }
                  else
                  {
                      alert("Please check the checkbox to confirm that you want to make a full refund");
                      return false;
                  }
              }
          }
